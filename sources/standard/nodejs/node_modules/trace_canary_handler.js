 ...
 *
 * @param {string} parameterName - Name of the SecureString parameter
 * @returns {Promise<Object>} Dictionary containing the parsed YAML data
 * @throws {Error} If the parameter cannot be retrieved or parsed
 */
async function loadYamlConfig(parameterName) {
    if (!parameterName) {
        throw new Error("CONFIG_SSM_PARAMETER_NAME must name the canary configuration parameter");
    }

    try {
        log.info(`Loading configuration from SSM parameter ${parameterName}`);
        const response = await ssmClient.send(new GetParameterCommand({
            Name: parameterName,
            WithDecryption: true
        }));
        const config = yaml.load(response.Parameter?.Value);

        if (!config || typeof config !== 'object') {
            throw new Error("SSM parameter configuration must contain a YAML object");
        }

        log.info("Configuration loaded successfully from SSM Parameter Store");
        return config;
    } catch (error) {
        if (error.name === 'ParameterNotFound') {
            log.error(`Configuration parameter not found: ${parameterName}`);
        } else if (error.name === 'AccessDeniedException') {
            log.error(`Access denied reading configuration parameter: ${parameterName}`);
        } else if (error.name === 'YAMLException') {
            log.error(`Configuration parameter ${parameterName} does not contain valid YAML`);
        } else {
            log.error(`Unable to load configuration parameter ${parameterName}: ${error.name || 'unknown error'}`);
        }
        throw error;
    }
}
...
const canaryHandler = async function() {
    ...
    try {
        ...
        const config = await loadYamlConfig(process.env.CONFIG_SSM_PARAMETER_NAME);
