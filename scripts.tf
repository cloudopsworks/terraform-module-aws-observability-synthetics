...
resource "local_file" "script_custom_node" {
  for_each = {
    for key, synth in local.synthetics : key => synth
    if upper(try(synth.canary.requests_type, "URL")) == "SCRIPT" && strcontains(try(var.request_scripts[each.value.canary.request_script_ref].runtime_version, synth.canary.runtime_version, local.default_runtime_version), "nodejs")
  }
...
resource "local_file" "script_custom_python" {
  for_each = {
    for key, synth in local.synthetics : key => synth
    if upper(try(synth.canary.requests_type, "URL")) == "SCRIPT" && strcontains(try(var.request_scripts[each.value.canary.request_script_ref].runtime_version, synth.canary.runtime_version, local.default_runtime_version), "python")
  }
  content         = try(var.request_scripts[each.value.canary.request_script_ref].content, each.value.canary.request_script)
